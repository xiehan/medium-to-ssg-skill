...
2. Extracts metadata from the page's structured data (`<script type="application/ld+json">`) with `<meta>`/`<title>` fallbacks: title, publish date, author, canonical URL, and the post's topic tags (read from the "Topic" pill buttons or the page's embedded data, since the personal-export format has no tags of its own).
...
- **Client-side-rendered embeds (tweets, gists, slides, etc.).** Medium injects social/embed cards (e.g. embedded tweets) with JavaScript *after* the page loads, so the server-rendered HTML the scraper fetches contains only an empty `<figure><div><div></div></div></figure>` placeholder — there is no `<img>` or `<iframe>` to capture. These come through as empty figures (the converter drops them). Recovering them would require a headless browser; that's out of scope for the scraper. If a post has missing embeds, capture it from the logged-in browser with the save-post bookmarklet, which records the fully-rendered DOM.
- **Images.** Left as remote `cdn-images-1.medium.com` / `miro.medium.com` URLs, matching the real export. `medium-to-ssg` downloads and self-hosts them later. Medium wraps each article image in an interactive "click to view full size" affordance — `<figure><div role="button">…<picture><source srcset><img></picture></div></figure>` — where the real source lives on the `<picture>`/`<source>` (and the bare `<img>` may have no `src`). The scraper rescues these (`_rescue_wrapped_images`) by lifting a clean `<img src>` out of the `role="button"` / `aria-hidden` wrapper **before** stripping interactive chrome, so the image survives instead of being deleted with the button.
